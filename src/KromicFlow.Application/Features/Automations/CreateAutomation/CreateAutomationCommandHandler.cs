...
internal sealed class CreateAutomationCommandHandler(
    IKromicFlowDbContext db,
    IAuditWriter auditWriter,
    IAutomationScopeService automationScopeService,
    IPlanEnforcementService planEnforcement) : IRequestHandler<CreateAutomationCommand, Result<AutomationDto>>
{
    public async Task<Result<AutomationDto>> Handle(CreateAutomationCommand request, CancellationToken cancellationToken)
    {
        var account = await db.InstagramAccounts
            .Include(x => x.User).ThenInclude(x => x.Plan)
        ...
        if (await db.UserRestrictions.AnyAsync(x => x.UserId == request.UserId && x.AutomationBlocked, cancellationToken))
            return Result<AutomationDto>.Failure("Automations are restricted for this user.");

        var count = await db.Automations.CountAsync(x => x.InstagramAccount.UserId == request.UserId, cancellationToken);
        var maxAutomations = await planEnforcement.GetMaxAutomationsAsync(request.UserId, cancellationToken);
        if (count >= maxAutomations)
            return Result<AutomationDto>.Failure("Plan automation limit reached.");
